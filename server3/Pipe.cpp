...
int Pipe::checkTarget(int now_line){
    if(!now_line) return 0;
    for(int i=1;i<PIPE_SIZE;i++){
        if(now_line==pipe_target[i]) {
            return i;  //ith target
        }
    }
    return 0;
...
int Pipe::number_create(int target_line,int user_id){
    for(int i=1;i<PIPE_SIZE;i++){
        if(!pipe_target[i]){
            pipe_target[i] = target_line;
            if(pipe(p_number[i])<0){
...
int Pipe::dup2_pipe(pipeState p_1,int user_id,pipeState p_2){
    ...
    if(p_1.type==pipeType::User){
        if(p_2.type==pipeType::STDOUT) dup2(p_1.num,STDOUT_FILENO);
        if(p_2.type==pipeType::STDIN) dup2(p_1.num,STDIN_FILENO);
        if(p_2.type==pipeType::STDERR) dup2(p_1.num,STDERR_FILENO);
    }
    return 1;
}
